...
download already feeds detention average daily population + removals; this adds zero fetch risk).
...
INTEGRITY CHECK (same philosophy as the average daily population dual-split): the three category
...
def criminality_block(rows):
    ...
    it). If a label appears in multiple blocks (average daily population-based vs point-in-time),
