#!/usr/bin/env python3
"""Measles cases — CDC measles surveillance page (weekly-updated HTML; no
...
CDC stops updating (the page was ~8 weeks behind by late Jul 2026), the card's
stale flag fires on the source's staleness, which is exactly the transparency
the product wants.

Historical annual counts (static, from the same CDC surveillance reporting)
...
# CDC annual confirmed measles cases (static context; current year comes live).
HISTORICAL = {
...
def parse_page(html):
    ...
    if not m:
        raise RuntimeError("CDC measles page sentence anchor not found (page restructured?)")
...
def main():
    ...
    out = {
        ...
        "source": {"name": "CDC — measles cases and outbreaks",
                   "url": "https://www.cdc.gov/measles/data-research/index.html"},
        "cadence": "Weekly", "stale_days": 45,
        "note": "Confirmed cases reported to CDC, year to date, dated by CDC's own 'as of' line — "
                "if this card flags stale, CDC has stopped updating. Measles was declared "
