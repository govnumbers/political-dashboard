#!/usr/bin/env python3
"""ICE detention — national average daily population (ADP), fiscal-YTD — ICE.
...
LAYOUT (FY26 vintage): per-facility ADP lives in the 'Facilities FY*' sheet
under a two-tier header — a group header row ('FY26 ADP: ...') with subheaders
below it (Level A–D, and Male/Female Crim/Non-Crim). National ADP = the sum of
...
as an extra field for card context — the headline stays ADP, as labelled, and
...
def adp_from_rows(rows):
    ...
    Sums Level A–D per-facility ADP; cross-checks against the Male/Female
    ...
    adp, n = colsum(lvl)
    if adp <= 0 or n < 20:
        raise RuntimeError(f"Facilities sheet: implausible ADP sum ({adp:.0f} from {n} facilities)")
    if len(crim) == 4:
        alt, _ = colsum(crim)
        if alt > 0 and abs(alt - adp) / adp > 0.01:
            raise RuntimeError(f"ADP cross-check failed: Level-split {adp:.0f} vs "
